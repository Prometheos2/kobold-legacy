    ...
    def die(self, cause="General incompetence", killer=None):
        ...
        for l in self.world.kobold_list:
            if l.carry == self:
                ...
                l.del_trait("carried")
        if (
            killer and killer != self and getattr(killer, "tribe", None)
    ...
    def leave(self, k, reform=True):
        if k in self.members:
            ...
            if k.hp > 0 and k.nick:
                action_queue.append(
                    ["delmember", self.chan, k.d_user_id]
            ...
            if not c.didturn:
                ...
                if target:
                    ...
                    if isinstance(target, Kobold) and target.hp <= 0:
                        targets.remove(target)  # target dead, don't beat a dead kobold
    ...
    def die(self, killer=None):
        ...
        if killer and isinstance(killer, Kobold) and killer.tribe:
            if self.faction != "none":
                killer.tribe.gain_heat(
                    self.heat, self.faction
...
async def confirm_prompt(chan, msg, confirm=None):
    ...
    while True:
        ...
        if res == None:
            break
        if str(res[1]) != "Lulumbus#2714" and (
            not confirm or res[1] == confirm
        ):  # Example: 'MyBot#1111'
