...
def tribe_name():
    try:
        with Path("data/tribe_names.txt").open() as f:
            temp_names = []
            for line in f:
                nam = line.strip("\n")
                nam = nam.capitalize()
...
class World:
    ...
    def find_tile_feature(self, dist, place, thing, feature, gen=False):
        ...
        for m in coords:
            if feature == "resources":
                search = [self.map[m].resources[d] for d in self.map[m].resources]
    ...
    def month_change(self):
        ...
        contained = []
        creatures = []
        diseases = [t for t in trait_data if trait_data[t].get("mc_disease", False)]
        console_crosspost = False
        ...
        for k in klist:
            ...
            trs = list(k.traits)
            for t in trs:
                if trait_data[t].get("mc_save_to_cure", False) and (
                    k.save(trait_data[t]["save_stat"]) + k.ap >= trait_data[t]["save"]
                ):
                    k.del_trait(t)
                    k.p(
        ...
        for c in creatures:
            ...
            if not c.owner or c.owner.hp <= 0:
                newowners = [k for k in klist if k.get_place() == p]
            ...
            if (p.z == 0 and "*grass" in c.diet) or c.has_trait("fed"):
                graze = True
            else:
                graze = False
            if not graze:
                ...
                for i in area:
                    ...
                    if i.type == "bait":
                        prio += 100
                    if i.type in ("food", "bait") and prio > fprio:
                        food = i
                        fprio = prio
            ...
            for a in list(c.traits):
                if trait_data[a].get("mc_reset", False):
                    c.del_trait(a)
        spoiling += [c for c in contained if c.perishable]
        for e in eggs:
            e.hatch()
        for i in spoiling:
            i.destroy("Spoiled")
...
class Tile:
    ...
    def minerals(self):
        ...
        reses = [p for p in possible if chance(possible[p])]
        while len(reses) < 4:
            reses.append(None)
    ...
    def get_border(self, d):
        ...
        return self.world.get_tile(borders[d][0], borders[d][1], borders[d][2])
    ...
    def finish_building(self, res, k):
        ...
        for n in needs:
            ...
            for b in gra:
                ...
                console_print(
                    "Could not consume " + b + " when building " + res["name"] + ".",
                    True,
                )
        if " Farm" in res["name"]:
            self.farm_cap = (
    ...
    def examine(self, me):
        ...
        msg += "\n\nItems here:\n"
        ks = [i.display() for i in self.items]
        msg += ", ".join(ks)
...
class Tribe:
    ...
    def defense(self):
        ...
        for b in self.buildings:
            res = find_building(b)
            if res.get("defense", 0) > 0 and (
                b not in self.building_health or self.building_health[b] >= 50
            ):
                d += res["defense"]
        return d
    ...
    def building_damage(self, build, dmg):
        ...
        if self.building_health[build] - dmg < 0:
            ...
            return
        if self.building_health[build] >= 50 and self.building_health[build] - dmg < 50:
            game_print(
                build
                + " took "
                + str(dmg)
    ...
    def examine(self, me):
        ...
        heats = [
            f + ": %.1f" % round(self.heat_faction[f], 1) for f in self.heat_faction
        ]
        msg += ", ".join(heats) + "\nWatchmen: "
        watch = []
        wdef = 0
    ...
    def community_effort(self):
        ...
        for x in tally:
            if not best or tally[x] > tally[best]:
                best = x
        ties = [x for x in tally if tally[x] == tally[best]]
        if len(ties) <= 0:
        ...
        if res:
            ...
            elif thing["name"] not in self.building_prog:
    ...
    def gain_heat(self, h, faction=None):
        ...
        for f in fs:
            ...
            if self.heat_faction[f] + h <= self.shc_faction[f]:
                ...
                continue
            if self.heat_faction[f] <= self.shc_faction[f]:
    ...
    def invasion(self, faction="Goblin"):
        ...
        dmgto = {}
        bolds = [
            k for k in self.kobolds if k.age >= 6 or not self.has_building("Nursery")
        ]
...
class Kobold:
    ...
    def max_cp(self):
        ...
        if self.tribe:
            if self in (self.tribe.chieftain, self.tribe.overseer):
                cp *= 2
    ...
    def worn_items(self):
        return [w for w in self.worns.values() if w]

    def familiar(self, r):
        res = find_research(r)
        if r in self.familiarity:
            return math.floor(self.familiarity[r] / res["diff"])
    ...
    def get_place(self):
        ...
        return (
            tribe if tribe and (self in tribe.kobolds or self in tribe.tavern) else tile
        )
    ...
    def get_chan(self):
        ...
        if self.commandedby and self.commandedby.get_place() == place:
    ...
    def char_info(self, k, pr=True):
        ...
        inv = [i.display() for i in self.items]
        isize = len(inv)
        if self.carry:
        ...
        if isize == 0:
            inv.append("Empty")
        msg += ", ".join(inv) + "\nWorn: "
        worn = [w.display() for w in self.worns.values if w]
        if len(worn) == 0:
            worn.append("None")
        msg += ", ".join(worn) + "\n\nStats:\n"
        statblock = [st + ": " + self.stat_str(st) for st in STATS]
        msg += " / ".join(statblock)
        msg += "\n\nSkills (total level: "
        sktotal = 0
        statblock = []
        ...
        if self.hp <= 0:
            ...
            if (
                ...
                inj = [
                    t
                    for t in trait_data
                    if trait_data[t].get("injury", False)
                    and (
            ...
            if (
                dmgtype in ["slashing", "piercing"]
                and self.save("con") < 8
                and not self.has_trait("infected")
            ):
            ...
            for t in trs:
                if trait_data[t].get("hurt_reset", False):
                    ...
                    trait_data[t].get("hurt_save_to_cure", False)
                    and self.save(trait_data[t]["save_stat"]) >= trait_data[t]["save"]
                ):
                    self.del_trait(t)
                    self.p(
                        "[n] has overcome their "
    ...
    def add_trait(self, t):
        ...
        if t in ("onearm", "noarms") and self.equip:
            self.equip = None
        for u in trs:
    ...
    def gain_xp(self, sk, exp):
        if (self.has_trait("inactive") or not self.nick) and self.skill[sk] >= 5:
            return
        if self.color in (STAT_COLOR[skill_data[sk]["stat"]], "silver"):
            exp *= 1.5
        if self.color == "brown":
            exp = exp * 1.2
...
class Party:
    ...
    @property
    def k_members(self):
        return [k for k in self.members if isinstance(k, Kobold)]

    @property
    def c_members(self):
        return [k for k in self.members if not isinstance(k, Kobold)]
        ...
        for k in mem:
            ...
            for i in k.items:
                ...
                if (
                    ...
                    and k.equip != i
                    and k.save("dex") < 15
                ) and chance((i.liquid_units / i.liquid_capacity) * 100):
...
class Item:
    ...
    def examine(self, me, multi=False):
        ...
        if self.type == "corpse":
            ...
            g = [y[0] + " x" + str(y[1]) for y in self.gain]
    ...
    def use(self, k):
        ...
        if self.name == "Manacite":
            ...
            sp = [
                ...
                if (
                    ...
                    and self.school in ("open", s["school"])
                    and s["level"] >= self.attunelevel - 1
                )
            ]
            newspell = choice(sp)
            if newspell:
            ...
            self.destroy("Manacite used")
            return True
        if self.name == "Charged Mana Cell":
        ...
        if self.name == "Inert Mana Cell":
            if k.mp_tax(10):
                ...
                return True
            return False
        if self.name == "Ant Pheromonal Gland":
            place = k.get_place()
            if isinstance(place, Tribe):
        ...
        if self.name == "Tin Rod":
            ...
            return True
        if self.name == "Flare Wand":
            tribes = [
                t for t in k.world.tribes if get_tri_distance(k.x, k.y, t.x, t.y) < 15
            ]
            if len(tribes) > 0:
                k.p(
    ...
    def map_render(self, k):
        ...
        for m in self.map:
            ...
            if self.map[m]["y"] > highy:
                highy = self.map[m]["y"]
        row = [" " for _a in range((((highx - lowx) + 1) * 2) + 1)]
        msg = [list(row) for _b in range((((highy - lowy) + 1) * 2) + 1)]
...
class Encounter:
    def __init__(self, world, tile, n, level, force=None):
        ...
        if (not force and chance(25)) or force == "kobold":
            ...
            if force == "kobold":
                type = "merchant"
            a = random.randint(2, 5) if type == "hunters" else 1
            for _b in range(a):
    ...
    def end(self):
        ...
        elif self.special == "Ant Queen" and self.place.dungeon:
            d = self.place.dungeon
            ow = self.world.get_tile(d.x, d.y, d.z)
...
class Creature:
        ...
        for _x in range(2):
            if chance(40 + self.stats["cha"]):
                if not summoned:
                    ...
                    summoned = True
                current = [c.name[-1] for c in self.encounter.creatures]
                a = ord("A")
    ...
    def attack(self, target):
        ...
        if isinstance(target, Kobold) and target.age < 6 and chance(95):
            adults = [k for k in target.party.k_members if k.age >= 6]
            tank = choice(adults)
            if tank:
...
def turn_traits(fighter):
    ...
    for t in trs:
        ...
        if (
            trait_data[t].get("turn_save_to_cure", False)
            and fighter.save(trait_data[t]["save_stat"]) >= trait_data[t]["save"]
        ):
            fighter.del_trait(t)
            fighter.p(
                "[n] has overcome their "
...
def cmd_farming(words, me, target):
    ...
    if not gotfarm:
        ...
        fs = [f + " x" + str(farms[f]) for f in farms]
        msg += ", ".join(fs) + "\n\nCrop progress:"
...
async def cmd_kobolds(words, me, chan):
    ...
    bolds = [k for k in me.world.kobold_list if k.get_place() == me.get_place()]
    for p in pages:
        l = []
        pdesc = ""
        pgn = 1
        for k in bolds:
            if p == "resources":
                ...
                n.extend([i.display() for i in k.items])
                m = (
                    k.display()
                    + "("
                    + str(len(k.items))
...
def cmd_bury(words, k, target):
    if target.name in ("Kobold Corpse", "Kobold Skeleton"):
...
async def cmd_pacify(words, k, target):
    if target.encounter.pacified:
        ...
        return False
    if not isinstance(target, Kobold) and target.language == "none":
...
def cmd_gift(words, k, target):
    ...
    if f:
        ...
        if v <= 0:
            ...
            elif "Common Language" not in k.tribe.research:
            ...
            if not target.has_trait("trader"):
                if not target.has_trait("broken"):
                    ...
                    if roll > 10:
                        ...
                        if target.encounter and (
                            len(target.encounter.creatures) > 1 and roll < 6
                        ):
                            k.p(
                                target.display()
...
async def cmd_lookall(words, me, chan):
    p = me.get_place()
    bolds = [k for k in me.world.kobold_list if k.get_place() == p]
    embeds = []
    for k in bolds:
        e = discord.Embed(
            type="rich",
...
def cmd_mount(words, k, me):
    ...
    if k.nick:
        if me.party and k not in me.party.members:
            ...
            me.party.join(k)
    elif me.party and k in me.party.members:
        me.party.leave(k)
...
def cmd_carry(words, me, k):
    ...
    if me.carry:
        ...
        return False
    if (isinstance(me, Creature) and me.owner.tribe != k.tribe) or (
    ...
    if k.carry:
        ...
        return False
    if k.nick and not k.has_trait("rescues"):
        me.p(
...
def cmd_give(words, me, i):
    ...
    if not target:
        ...
        if not target:
            ...
            return False
        if "haul" not in target.training:
...
def cmd_comfort(words, me, target):
    saves = [
        t for t in target.traits if trait_data[t].get("comfort_save_to_cure", False)
    ]
...
def cmd_heal(words, me, target):
    ...
    if target.hp < target.max_hp or cond:
        ...
        if bestmed:
            ...
            for t in target.traits:
                if trait_data[t].get("heal_save_to_cure", False) and (
...
def cmd_attack(words, me, target):
    ...
    if isinstance(me, Creature):
        me.attack(target)
        return True
    if me.equip and me.equip.type in ["melee", "ranged", "magic", "finesse"]:
...
def cmd_equip(words, me, target):
    ...
    if me.has_trait("noarms"):
        ...
        return False
    if target.twohands and me.has_trait("onearm"):
...
def cmd_leave(words, me, target):
    ...
    if dgn and "Dungeon Exit" in tribe.special:
        ...
        me.get_place().examine(me)
        me.broadcast(
            "[n] and their party emerge from the " + dungeon_data[dgn.d]["name"] + "."
        )
        return True
...
def cmd_enter(words, me, target):
    ...
    if tribe:
        ...
        if not dgn:
            ...
            if dungeon_data[dgn.d].get("faction", None) and (
                me.tribe and me.tribe.shc_faction[dungeon_data[dgn.d]["faction"]] < 1
            ):
    ...
    for c in e.creatures:
        for t in c.traits:
            if not trait_data[t].get("turn_block", False):
                ao.append(c)  # noqa: PERF401
                # sorry, chief; no clue how to make a comprehension here
    for k in me.party.members:
...
def forage(me, lm=None, dgn=None, mm=None):
    ...
    if dgn:  # must be dungeon
        ...
        for i in item_data:
            if (
                i.get("forage", None)
                and z >= i["forage"]["level"][0]
                and (i["forage"]["level"][1] == -1 or z <= i["forage"]["level"][1])
            ):
...
def cmd_attune(words, me, target):
    ...
    if not gem:
        ...
        return False
    al = 1 if target.school not in (gem.school, "open") else target.attunelevel + 1
...
async def cmd_use(words, me, chan):
    ...
    if target in ("Hot Spring", "Artificial Hot Spring"):
        if me.ap_tax(1):
...
def cmd_extend(words, me, target):
    p = me.get_place()
    tribe = None
    if isinstance(p, Tribe):
        tribe = p
...
def cmd_farm(words, k, target):
    ...
    if not tribe:
        ...
        return False
    if f["name"] not in tribe.farmable:
...
def cmd_chop(words, k, target):
    ...
    while t.mineprog["w"] >= 100:
        ...
        if chance(10 + t.stability - k.skmod("woodcutting")):
            bolds = [l for l in k.world.kobold_list if l.get_place() == t]
            target = choice(bolds)
            if target:
...
async def cmd_info(words, user, chan, w):
    ...
    info.extend(
        [[r, "Building"] for r in building_data if words[1] in r["name"].lower()]
    )
    info.extend(
        [[r, "Research"] for r in research_data if words[1] in r["name"].lower()]
    )
    info.extend([[r, "Spell"] for r in spell_data if words[1] in r["name"].lower()])
    info.extend(
        [[r, "Creature"] for r in creature_data if words[1] in r["name"].lower()]
    )
    for i in item_data:
    ...
    info.extend([[r, "Craft"] for r in craft_data if words[1] in r["result"].lower()])
    info.extend([[r, "Landmark"] for r in landmark_data if words[1] in r.lower()])
...
def cmd_quit(words, me, p, force=False):
    ...
    if me.party:
        ...
        if me.party.owner == me:
            eligible = [m for m in me.party.k_members if m.nick]
...
def get_newbold(user, chan, w, test=False, nt=False):
    ...
    for p in playerdata:
        ...
        if len(playerdata[p].get("selection", {})) > 0:
            inselection.append(playerdata[p]["selection"].copy())
...
async def multi_select(
    ...
    if landmarks:
        tile = me.world.get_tile(me.x, me.y, me.z)
        targets.extend([l for l in tile.special if search in l.lower()])
    if not chan:
        if len(targets) == 0:
            return None
...
async def cmd_unlockall(words, user, chan, w):
    for t in w.tribes:
        t.research.extend(
            [r["name"] for r in research_data if r["name"] not in t.research]
        )
        t.buildings.extend(
            [
                b["name"]
...
async def confirm_prompt(chan, msg, confirm=None):
    ...
    with contextlib.supress(discord.HTTPException):
        ...
        if not me.tribe or me != incharge:
            ...
            if a:
                if not a.get("task", False):
                    ...
                    return None
                me.tribe.tasks.extend([words[2], None, None, None, True])
...
async def cmd_routine(words, user, chan, w):
    ...
    if words[1] == "run":
        ...
        if not k:
            await chan.send("Kobold not found.")
            return None
        if k.nick and k != me and (not me.tribe or me.tribe.overseer != me):
            await chan.send(
        ...
        if words[2] in get_pdata(pid, "routines", {}):
            for a in playerdata[pid]["routines"][words[2]]:
                ...
                if not await handle_message(m):
                    await chan.send(
                        "Could not complete the routine. Aborted at action: `" + a + "`"
                    )
                    return None
        ...
        await chan.send("Routine '" + words[2] + "' not found.")
        return None
    if words[1] in ["fo", "final", "finalorders", "final_orders"]:
        if len(words) < 3:
        ...
        if not k:
            await chan.send("Kobold not found.")
            return None
        if not k.orders and k != me:
            await chan.send(
                "Can't set final orders for this kobold (they have orders disabled)."
            )
            return None
        if k != me and (not me.tribe or k.tribe != me.tribe or me.tribe.overseer != me):
        ...
        await chan.send("Routine '" + words[2] + "' not found.")
        return None
    if "rediting" not in playerdata[pid] or playerdata[pid]["rediting"] is None:
...
def load_game(path="klsave"):
    ...
    for w in ws:
        ...
        for m in w.map:
            allitems.append(w.map[m].items.copy())
        ...
        if (
            ...
            if k.nick:
                if (
                    ...
                    return None
                if not k.orders and not k.has_trait("inactive"):
                    await chan.send(
            ...
            message.content = " ".join(words)
        if len(words) == 0:
            return None  # clearly not a command
            ...
            if a:
                ...
                if len(words) - 1 < argsneeded and not console:
                    ...
                    if user and (
                        ...
                        return None  # ignore channels that aren't game channels
                    if not me:
