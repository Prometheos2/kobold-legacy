...
ROLENAMES = {"brown": "Mudscale", "red": "Bloodscale", "yellow": "Goldscale", "green": "Jadescale", "blue": "Silkscale",
             "white": "Marblescale", "black": "Coalscale", "orange": "Copperscale", "purple": "Violetscale",
             "silver": "Silverscale"}
    ...
    def month_change(self):
        ...
        for t in self.tribes:
            ...
            for i in list(t.items + t.kennel_items):
                ...
                if i.perishable:
                    spoiling.append(i)
                if i.liquid and i.liquid != "Water" and not liquid_data[i.liquid].get("potion", False) and liquid_data[
                    i.liquid].get("drinkable", True):
                    liqs += i.liquid_units
                if i.inert:
                    i.inert = False
        ...
        for k in klist:
            ...
            if k.hp > 0 and k.tribe:
                ...
                if k.nick and time.time() - k.lasttime >= 86400 and not k.has_trait("inactive"):
                    ...
                    k.p("<@!" + str(
                        k.d_user_id) + "> You have not submitted a command in the last 24 hours, so you are inactive. Anyone can now order you as though you were nameless. You can remove this status by submitting any command.")
        ...
        for c in creatures:
            ...
            if not c.owner or c.owner.hp <= 0:
                ...
                if c.owner:
                    ...
                    ) + " senses that their master is not coming back and finds a new bond with " + c.owner.display() + ".",
                               c.get_chan())
            ...
            if food or graze:
                ...
                c.hp_gain(1)
                if "domestic" in c.training and not isinstance(p,
                                                               Tribe) and "Pasture" in p.special and "Egg" in c.products:
                    spawn_item("Egg", p)
            else:
                game_print(
...
class Tile:
    ...
    def landmarks(self):
        ...
        for l in landmark_data:
            if self.z >= landmark_data[l]["level"][0] and (
                    self.z <= landmark_data[l]["level"][1] or landmark_data[l]["level"][1] == -1):
    ...
    def examine(self, me):
        ...
        if "Road" in self.special or "Paved Road" in self.special:
            roadirs = []
            for d in DIR_FULL:
                if ("Road" in self.get_border(d).special or "Paved Road" in self.get_border(d).special) and not \
                self.blocked[d]:
...
class Tribe:
    ...
    def get_available_builds(self, k=None):
        ...
        for r in building_data:
            ...
            if r["name"] in self.buildings and not r.get("repeatable", False):
                good = False
            if r.get("landmark", False) and r["name"] in self.world.get_tile(self.x, self.y,
                                                                             self.z).special and not r.get("repeatable",
                                                                                                           False):
                good = False
    ...
    def invasion(t, faction="Goblin"):
        ...
        if t.has_building("Marble Statues") and faction in ["Goblin", "Human", "Elf", "Dwarf"] and t.heat_faction[
            faction] > 5:
...
class Kobold:
    ...
    def hp_tax(self, n, cause, killer=None, dmgtype="bludgeoning"):
        ...
        if self.hp <= 0:
            ...
            if dmgtype not in ["poison", "arcane"] and n >= min(10, math.ceil(self.max_hp / 2)) and self.save(
                    "con") < n + 5:
                inj = []
                for t in trait_data:
                    if trait_data[t].get("injury", False) and (
                            not self.has_trait(t) or not self.has_trait(trait_data[t].get("worse", t))):
                        inj.append(t)
                injury = choice(inj)
                if injury:
            ...
            p = self.get_place()
            if isinstance(p,
                          Tribe) and self in p.tavern and cause != "Dangerous drink" and not self.nick and not self.party:
    ...
    def breed(self, partner, force=False, pullout=False):
        ...
        if force or chance(ch):
            ...
            elif (self.has_trait("nonbinary") or partner.has_trait("nonbinary") or self.male != partner.male) and not (
                    self.has_trait("infertile") or partner.has_trait("infertile")):
...
class Party:
    ...
    def stealth_roll(self, encounter, bonus=0, me=None, aps=0):
        if not encounter.hostile:
            return
        if self.owner.tribe and encounter.creatures[0].faction in self.owner.tribe.shc_faction and \
                self.owner.tribe.shc_faction[encounter.creatures[0].faction] < 1:
            encounter.hostile = False
            self.owner.p(
                "The " + encounter.creatures[
                    0].faction + " faction has called a truce with the kobolds, so they are not hostile.")
            return
        stealth = bonus
    ...
    def move(self, x, y, z, cost):
        ...
        for k in mem:
            ...
            for i in k.items:
                if i.name == "Crude Map":
                    i.map_update(k)
                if i.liquid and not i.sealable and i.liquid_units > 0 and not liquid_data[i.liquid].get("powder",
                                                                                                        False) and k.equip != i and k.save(
                        "dex") < 15:
...
class Item:
    def __init__(self, name, num=1):
        for i in item_data:
            ...
            if i["name"] == name:
                ...
                break
        self.num = num
        if self.num > self.stack:
            self.num = max(self.stack, 1)
        self.quality = 0
        self.attunelevel = 0
    ...
    def move(self, to, tumble=False):
        ...
        if self.num > 0:
            self.place = to
            if isinstance(to, Item):
                to.contains = self
            elif isinstance(to, list):
                to.append(self)
                self.place = None
            else:
                to.items.append(self)

    def destroy(self, cause="Unknown"):
        if self.place:
            if isinstance(self.place, Item):
                self.place.contains = None
    ...
    def use(self, k):
        ...
        if self.name == "Default":
            ...
            if not k.has_trait("fed"):
                ...
                k.p("[n] has already eaten recently.")
            ...
            if k.has_trait("oneleg"):
                ...
                k.p("There's no need for this. Yet.")
        elif self.name == "Stone Tablet":
            if self.note == "":
                k.p("Nothing is written here. Type !write <text> to write something.")
            else:
                k.p(self.note)
            return True
            ...
            if not k.has_trait("manacite"):
                ...
                if k.hp <= 0:
                    ...
                    return True
            for s in spell_data:
                if s["name"] not in k.spells and s["level"] <= (k.skill["sorcery"] / 2) and "arcane" in s[
                    "spell_class"]:
            ...
            if len(tribes) > 0:
                ...
                for t in tribes:
                    ...
                    if dstr != "":
                        game_print(
                            "A puff of smoke materializes in the middle of the den, showing brief visions of " + k.display(
                            ) + ". Everyone who witnesses is magically made aware of the signal's relative origin: " + dstr,
                            t.get_chan())
                self.magic_item_use(
    ...
    def hatch(self):
        if not self.kobold:
            if self.place:
                ...
                if isinstance(self.place, Kobold):
                    t = self.place.tribe
                elif isinstance(self.place, Tribe):
                    t = self.place
                else:  # see which kobolds are present, pick one of them
                    bolds = []
                    for k in self.place.world.kobold_list:
                        if k.tribe and k.get_place() == self.place:
                            bolds.append(k)
                    t = choice(bolds).tribe
                if t:
                    self.kobold = Kobold(t)
                else:
                    self.kobold = Kobold(self.place.world.tribes[0])
                if not isinstance(t, Tribe):
                    self.kobold.tribe = None
                self.kobold.age = 0
                self.kobold.random_stats()
                for st in self.kobold.s:
                    self.kobold.s[st] = 0
        ...
        if self.kobold not in self.kobold.world.kobold_list:
            self.kobold.world.kobold_list.append(self.kobold)
    ...
    def map_merge(self, other):
        combined = {}
        for m in self.map:
            combined[m] = dict(self.map[m])
        for m in other.map:
            combined[m] = dict(other.map[m])
        for m in combined:
            if m in self.map and m in other.map:
                if self.map[m]["symbol"] != "O":
                    combined[m]["symbol"] = self.map[m]["symbol"]
                if other.map[m]["symbol"] != "O":
                    combined[m]["symbol"] = other.map[m]["symbol"]
                for d in DIR_FULL:
                    if self.map[m][d] and other.map[m][d]:
                        combined[m][d] = True
                    else:
                        combined[m][d] = False
    ...
    def map_update(self, k):
        ...
        if isinstance(t, Tribe):
            t = k.world.get_tile(t.x, t.y, t.z)
        if t.get_tribe():
            self.map[m]["symbol"] = "T"
        elif t.camp:
            self.map[m]["symbol"] = "C"
        elif len(t.special) > 0:
            self.map[m]["symbol"] = landmark_data[t.special[0]]["mark"]
        else:
            self.map[m]["symbol"] = "O"
        for d in DIR_FULL:
...
class Encounter:
        ...
        if (not force and chance(25)) or force == "kobold":
            ...
            for b in range(a):
                ...
                if type == "child":
                    ...
                    if type == "hunters":
                        w = choice(
                            ["Stone Spear", "Stone Hammer", "Stone Knife", "Sling", "Stone Spear", "Stone Hammer",
    ...
    def start(self, party):
        if party.owner.tribe and not isinstance(self.creatures[0], Kobold) and self.creatures[
            0].faction in party.owner.tribe.shc_faction and party.owner.tribe.shc_faction[
            self.creatures[0].faction] < 1:
    ...
    def disengage(self, party):
        for k in party.members:
            ts = list(k.traits)
            for t in ts:
                if trait_data[t].get("end_combat", False):
                    k.traits.remove(t)
        while party in self.engaged:
            self.engaged.remove(party)
    ...
    def pac_check(self):
        ...
        for c in self.creatures:
            if not c.has_trait("pacified") and not c.has_trait("sleep"):
                pac = False
        if pac:
            for p in self.engaged:
    ...
    def enemy_turn(self, party):
        ...
        for k in party.members:
            ...
            trs = list(k.traits)
            for t in trs:
                if trait_data[t].get("turn_reset", False):
                    k.del_trait(t)
        for c in self.creatures:
            c.didturn = False
            turn_traits(c)
            if not c.didturn:
                ...
                if target:
                    ...
                    if isinstance(target, Kobold) and target.hp <= 0:
...
class Creature:
    ...
    def get_place(self):
        if self.encounter:
            return self.encounter.place
        elif self.party:
            return self.party.owner.get_place()
        else:
            for t in self.world.tribes:
                if self in t.kennel:
                    return t
            for m in self.world.map:
                if self in self.world.map[m].pasture:
                    return self.world.map[m]
        return None
    ...
    def add_trait(self, trait):
        if trait in self.trait_immune:
            return
        if trait not in self.traits:
            self.traits.append(trait)
        if trait_data[trait].get("contract_msg", None):
            self.p(trait_data[trait]["contract_msg"])
    ...
    def die(self, killer=None):
        ...
        if isinstance(p, Tribe) and self in p.kennel:
            p.kennel.remove(self)
        elif isinstance(p, Tile) and self in p.pasture:
            p.pasture.remove(self)
        ...
        if killer and isinstance(killer, Kobold) and killer.tribe:
            if self.faction != "none":
                # there was a really long chain of variables here but it's gone now
    ...
    def summon(self, enemy):
        ...
        if chance(40 + self.stats["cha"]):
            ...
            game_print("Nothing answered the call.", self.get_chan())
...
async def cmd_cast(words, me, chan):
    ...
    if targ == "kobold" or targ == "living":
        ...
        if isinstance(target, Kobold) and me != target and target.nick and sp.get("require_consent", False):
            ...
            if not await confirm_prompt(ch, me.display() + " wants to cast " + sp[
                "name"] + " with you as the target. Your consent is required. Do you accept?",
                                        discord.utils.get(guild.members, id=target.d_user_id)):
                me.p("Action aborted; consent not given.")
                return False
    if targ == "item":
        target = await multi_select(chan, args[sp.get("targ_arg", len(args) - 1)].lower(), me,
                                    sp.get("start_in_inv", True), sp.get("place", "any"), sp.get("target_type", None))
        if not target:
            return False
...
async def cmd_pacify(words, k, target):
    if target.encounter.pacified:
        ...
        if target.stats["int"] < 6 and target.companion:
            bait = await multi_select(discord.utils.get(guild.channels, name=k.get_chan()), "", k, place="inv",
    ...
    if not f:
        ...
        if roll < diff:
            ...
            if k.tribe.heat_faction[f] <= 0:
                ...
                k.p("Thanks to " + n[
                    2].display() + "'s negotiation skills, the " + bn + " is willing to hear them out, and they've made some significant progress. Heat has decreased, but the " +
...
async def cmd_feeder(words, k, target):
    if words[1].lower() == "add":
        ...
        scope = "kennel"
    item = await multi_select(discord.utils.get(guild.channels, name=k.get_chan()), "", k, place=scope,
                              type="bait/food/material")
    if item:
        if scope == "any":
        ...
        if not isinstance(p, Tribe):
            for l in p.special:
                if l not in landmark_data:
                    continue
                if landmark_data[l].get("liquid_source", None) and liq.lower() == landmark_data[l][
                    "liquid_source"].lower():
...
def cmd_hide(words, me, target):
    ...
    me.p(
        "[n] looks for a good hiding place to wait out the raid. They estimate that their survival chances are about " + str(
            100 - me.hiding) + "%.")
...
async def cmd_use(words, me, chan):
    ...
    if isinstance(target, Item):
        if target.hp < 0:
            ...
            conf = await confirm_prompt(chan, "This item is dangerous; it will cost HP to use. Do you wish to proceed?",
                                        user)
            if not conf:
                return False
    ...
    if c:
        ...
        if c.get("liquid", False):
            ...
            for i in me.items + me.get_place().items:
                if i.type == "container" and i.liquid == c["result"] and i.liquid_units + c.get("amount",
                                                                                                1) <= i.liquid_capacity:
                    container = i
                    break
            if not container:
                for i in me.items + me.get_place().items:
                    if i.type == "container" and not i.liquid and i.liquid_units + c.get("amount",
                                                                                         1) <= i.liquid_capacity:
                        container = i
            if not container:
        ...
        if good == "good":
            if c.get("mana", 0) > 0:
                ...
                cost = c["work"]
                if c.get("blast_processing", False) and isinstance(place, Tribe) and place.has_building(
                        "Blast Furnace"):
                    cost -= 1
...
def cmd_breed(words, me, k):
    ...
    if good:
        if me.ap < 2 or k.ap < 2:
            ...
            me.ap_tax(2)
            k.ap_tax(2)
            if len(words) > 2 and words[2].lower() in ["restrain", "restraint", "pullout", "careful", "nobaby",
                                                       "noeggs", "noegg", "nobabies"]:
                pullout = True
            else:
                pullout = False
...
def cmd_study(words, k, target):
    ...
    if good != "good":
        k.p("Can't study " +
            r[
                "name"] + ". The technology must be available to the tribe or another kobold must be very familiar with it.")
        return False
    ...
    for k in w.kobold_list:
        if not k.nick:
            if k.d_user_id and user.id == k.d_user_id:  # returning player who left/quit
                nameless = [k]
                break
            elif k.tribe and k.tribe.id not in exed and not k.d_user_id and k.age >= 6 and not nt and str(
                    k.id) not in sel and not k.has_trait("locked"):
...
async def cmd_sp(words, user, chan, w):
    ...
    spe = get_pdata(user.id, "sp_earned", 0)
    await chan.send(
        "You currently have " + str(sp) + " Soul Points. You have earned " + str(spe) + " Soul Points to date.")
    return False
...
async def cmd_name(words, user, chan, w, wand=None):
    ...
    if not wand:
        ...
        if nid is not None:
            if len(sel) > 1:
                ...
                await chan.send(
                    "That kobold is no longer available. You have been refunded 10 Soul Points (current balance: " + str(
                        playerdata[str(user.id)]["sp"]) + ")")
            else:
    ...
    if newbold.party:
        ...
        await chan.send(
            "You have joined the game! Your current channel is #" + newbold.party.get_chan() + " on the server.")
    p = newbold.get_place()
    if isinstance(p, Tribe):
...
async def cmd_kvar(words, user, chan, w):
    ...
    if k:
        ...
        if words[2] in k.s:
            ...
            setattr(k, words[2], v)
        await chan.send("Set attribute " + words[2] + " of kobold " + k.get_name() + " to " + str(v) + " (type " + str(
            type(v)) + ")")
    else:
        await chan.send("Kobold " + words[1] + " not found")
...
async def cmd_tvar(words, user, chan, w):
    ...
    if tribe:
        ...
        await chan.send("Set attribute " + words[2] + " of tribe " + tribe.name + " (" + str(tribe.id) + ") to " + str(
            v) + " (type " + str(type(v)) + ")")
    else:
        await chan.send("Tribe " + words[1] + " not found")
...
async def edit_wanderer(chan, user=None):
    ...
    if not k:
        ...
        ), "nick": "???", "sex": sex, "str": 10, "dex": 10, "con": 10, "int": 10, "wis": 10, "cha": 10,
            "color": "brown"}
        ...
        if not me.tribe or me != incharge:
            ...
            if s is not None:
                ...
                if words[3] not in breaks:
                    await chan.send(
                        "Invalid type '" + str(words[3]) + "'. Must be one of the following: " + ", ".join(breaks))
                    return False
                ...
                while True:
                    ...
                    for k in bs:
                        ...
                        if k.ap >= act.get("cost", 0):
                            if not act.get("skill", None) or k.skmod(act["skill"]) + k.smod(
                                    skill_data[act["skill"]]["stat"]) > best:
                    ...
                    try:
                        if d[1]:
                            if d[1] == "times" and times >= int(d[2]):
                                break
                            elif hasattr(t, d[1]) and isinstance(getattr(t, d[1]), int) and getattr(t, d[1]) >= int(
                                    d[2]):
                                break
                            elif d[1] == "farm_prog" and d[3] in tile.farming_prog and tile.farming_prog[d[3]] >= int(
                                    d[2]):
                                break
...
async def cmd_verify(words, member, chan, w):
    ...
    await newchan.send("<@!" + str(
        member.id) + "> Welcome! Make yourself at home, be sure to have a look at the <#918263140100239381> and <#918268315980402698>, and type `!join` in any channel when you're ready to join!")
    return True
...
async def on_member_join(member):
    ...
    await newchan.send("<@!" + str(
        member.id) + "> Hey! Before you can begin your legacy, you need to let me know you're a real person by typing `!verify` in the chat here. If that doesn't work or something is wrong, please ping/DM the dev (orange name).")
...
async def handle_message(message, num=1):
    ...
    try:
        ...
        if message.content[0] == ">" and (
                ">!" in words[0] or "> !" in message.content or ">-" in words[0] or "> -" in message.content):
            if words[0] != ">":
        ...
        if message.content[0] == ">" and cname != user.id and ("tribe" in cname or "party" in cname):
            ...
            if not k:
                ...
                if not k:
                    await chan.send(
                        "Kobold/creature '" + s + "' not found. Make sure you've spelled their name correctly and they're in the same area as you.")
                    return
            if k.nick:
                if not me.tribe or not k.tribe or (not k.tribe.overseer == me and (
                        not k.party or k.party != me.party or me.party.owner != me or isinstance(place, Tribe))):
                    await chan.send(
        ...
        if words[0][0] == "!" or (words[0][0] == "-" and cname != "changelog"):
            ...
            if a:
                ...
                if len(words) - 1 < argsneeded and not console:
                    ...
                    elif ordering and me.nick and (
                            time.time() - me.lasttime < 300 or a.get("cost", 0) > 0 or a.get("cp",
                                                                                             0) > 0) and not me.has_trait(
                            "inactive"):
                        await chan.send(
                    ...
                    elif ordering and isinstance(me, Kobold) and a.get("command_by", None) and getattr(me.tribe,
                                                                                                       a["command_by"],
                                                                                                       None) != oldme:
                    ...
                    elif ordering and isinstance(me, Creature) and (
                            a.get("animal_training", None) and a["animal_training"] not in me.training and a[
                        "animal_training"] != "any" and me.stats["int"] < 6):
                    ...
                    elif a.get("chieftain", False) and isinstance(me, Kobold) and (
                            not me.tribe or me.tribe.chieftain != me or ordering):
                        await chan.send("Only the chieftain can do this.")
                    elif a.get("overseer", False) and isinstance(me, Kobold) and (
                            not me.tribe or (me.tribe.overseer and me.tribe.overseer != me) or ordering):
                        await chan.send("Only the overseer can do this.")
                    elif not a.get("anywhere", False) and not a.get("overworld", False) and (
                            "tribe" not in me.get_chan() or me in place.tavern):
                        await chan.send(
                    ...
                    elif ordering and (a.get("cost", 0) > 0 or a.get("cp", 0) > 0) and oldme.cp < a.get("cp",
                                                                                                        1) and me.age >= 6:
                        await chan.send("You don't have enough CP to command a kobold to do this. (have " + str(
                            oldme.cp) + ", need " + str(a.get("cp", 1)) + ")")
                    else:
                ...
                if targ == "item":
                    ...
                    if user:
                        target = await multi_select(chan, search, me, a.get("start_in_inv", True),
                                                    a.get("place", "any"), a.get("target_type", None),
                                                    ordering=ordering)
                    else:
                ...
                if user:
                    if a.get("confirm_prompt", None):
                        ...
                        if not await confirm_prompt(chan,
                                                    "This action could potentially kill " + me.display() + " at their current HP. Proceed anyway?",
                                                    user):
                            await chan.send("Action aborted.")
                            return
                    elif me and me.has_trait("tool_broke"):
                        me.del_trait("tool_broke")
                        if not await confirm_prompt(chan, me.display() + "'s tool broke recently. Proceed anyway?",
                                                    user):
                            await chan.send("Action aborted.")
                            return
        ...
        if user:
            await log_exception(
                "Player: " + str(user) + ", channel: " + str(cname) + ", command: `" + message.content + "`")
        else:
            await log_exception("Player: " + me.get_name() + ", channel: " + str(
                cname) + ", command: `" + message.content + "` (final orders)")
