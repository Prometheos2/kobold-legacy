...
class Encounter:
    def __init__(self, world, tile, n, level, force=None):
        ...
        if (not force and chance(25)) or force == "kobold":
            ...
            if force:
                ...
                for c in creature_data:
                    if c["level"][0] <= level and c["level"][1] >= level and (
                            c["cr"] * 2 < n or c["cr"] <= 1):
    ...
    def start(self, party):
        if party.owner.tribe and not isinstance(
                self.creatures[0], Kobold) and self.creatures[0].faction in party.owner.tribe.shc_faction and party.owner.tribe.shc_faction[self.creatures[0].faction] < 1:
